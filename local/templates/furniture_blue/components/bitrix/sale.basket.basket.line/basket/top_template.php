...
<? if (!$arResult["DISABLE_USE_BASKET"])
		{
          ...
          foreach ($basket->getQuantityList() as $item) {
              $countTotal += (int)$item;
          }
		?>
            <a href="<?= $arParams['PATH_TO_BASKET'] ?>" class="header__basket">
                <img src="<?= SITE_TEMPLATE_PATH?>/images/icons/basket.svg" alt="">
                <?php if ($countTotal) :?>
                    <span id="total-basket-viget-count" class="in-basket"><?= $countTotal?></span>
                <?php endif;?>
            </a>
