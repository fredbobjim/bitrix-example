	...
	?>
        <section class="checkout">
            <div class="o-limit-x">
                ...
                <form class="validation flex justify-between gap-6 flex-col md:flex-row" action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
                    <?
                    ...
                    <div id="bx-soa-order" class="flex justify-between gap-6 flex-col md:flex-row">
                        <!--	MAIN BLOCK	-->
                        <div class="flex-1">
                            <div id="error-message" style="color:red"></div>
                            <div id="bx-soa-properties" data-visited="false" class="checkout__block">
                                <h4>Покупатель</h4>
                                <div class="flex flex-wrap gap-4 pb-2 validation checkout-form mir-input__wrap">
                                    <input type="text" id="soa-property-1" name="ORDER_PROP_1" class="w-full letters-only" placeholder="Имя"
                                           value="<?= $arResult['ORDER_PROP']['PRINT'][1]['VALUE']?>">
                                    <input type="email" class="w-full md:w-auto" id="soa-property-2" name="ORDER_PROP_2" placeholder="Email"
                                           value="<?= $arResult['ORDER_PROP']['PRINT'][2]['VALUE']?>">
                                    <input type="text" class="w-full md:w-auto phone-mask" id="soa-property-3" name="ORDER_PROP_3"  placeholder="Телефон"
                                           value="<?= $arResult['ORDER_PROP']['PRINT'][3]['VALUE']?>">
                                </div>
                            </div>
                            ...
                            <div class="checkout__block">
                                ...
                                <div class="flex gap-3 flex-wrap">
                                    ...
                                    <?php foreach ($arResult['PAY_SYSTEM'] as $key => $paySystem) :?>
                                        <div data-paysytem-id="<?= $paySystem['ID']?>" class="checkout__pay_item<?php if ($key == 0) :?> checkout__pay_active<?php endif;?>">
                                            <div class="checkout__pay_img">
                                                <?php
                                                if ($paySystem['ID'] == "2")
                                                    $paySystemLogo = SITE_TEMPLATE_PATH.'/images/visa.svg';
                                                if ($paySystem['ID'] == "3")
                                                    $paySystemLogo = SITE_TEMPLATE_PATH.'/images/sbp.svg';
                                                ?>
                                                <img src="<?= $paySystemLogo?>" alt="">
                                            </div>
                                            <p><?= $paySystem['NAME']?></p>
                                        </div>
                                    <?php endforeach;?>
...
<!--                                    <div class="bx-soa-section-title-container">-->
<!--                                        <h2 class="bx-soa-section-title col-sm-9">-->
<!--                                            <span class="bx-soa-section-title-count"></span>--><?php //=$arParams['MESS_PAYMENT_BLOCK_NAME']?>
<!--                                        </h2>-->
<!--                                        <div class="col-xs-12 col-sm-3 text-right"><a href="" class="bx-soa-editstep">--><?php //=$arParams['MESS_EDIT']?><!--</a></div>-->
<!--                                    </div>-->
<!--                                    <div class="bx-soa-section-content container-fluid"></div>-->
<!--                                </div>-->
                            <? endif ?>
...
<!--                                <div class="bx-soa-section-title-container">-->
<!--                                    <h2 class="bx-soa-section-title col-sm-9">-->
<!--                                        <span class="bx-soa-section-title-count"></span>--><?php //=$arParams['MESS_BUYER_BLOCK_NAME']?>
<!--                                    </h2>-->
<!--                                    <div class="col-xs-12 col-sm-3 text-right"><a href="" class="bx-soa-editstep">--><?php //=$arParams['MESS_EDIT']?><!--</a></div>-->
<!--                                </div>-->
<!--                                <div class="bx-soa-section-content container-fluid"></div>-->
<!--                            </div>-->
                            ...
                            <? if ($arParams['BASKET_POSITION'] === 'after'): ?>
                                <!--	BASKET ITEMS BLOCK	-->
                                <div id="bx-soa-basket" data-visited="false" class="bx-soa-section bx-active 222">
                                    <div class="bx-soa-section-title-container">
                                        <h2 class="bx-soa-section-title col-sm-9">
                                            <span class="bx-soa-section-title-count"></span><?=$arParams['MESS_BASKET_BLOCK_NAME']?>
                                        </h2>
                        ...
                        <div class="checkout__info">
                            <div class="checkout__info-content">
                              <?php foreach ($arResult['BASKET_ITEMS'] as $key => $item) {
                                $count += (int)$item['QUANTITY'];
                              }
                              ?>
                                <h4><?= $count?> товара в заказе</h4>
                                <div class="checkout__table">
                                    ...
                                    <?php foreach ($arResult['BASKET_ITEMS'] as $key => $item) :?>
                                      ...
                                      <?php
                                        $count += (int)$item['QUANTITY'];
                                      ?>
                                        <div class="checkout__tr grid grid-cols-12">
                                            <div><?= $key + 1?></div>
                                            <div class="col-span-6"><?= $item['NAME']?></div>
                                            <div class="col-span-3"><?= $item['QUANTITY']?> <?= $item['MEASURE_NAME']?></div>
                                            <div class="col-span-2"><?= $item['SUM_BASE_FORMATED']?></div>
                                        </div>
                                    <?php endforeach;?>

                                </div>
                                <div class="checkout__footer">
                                    <a id="pay_order_button" href="javascript:void(0)" class="btn-filled">Оплатить <?= $arResult['JS_DATA']['TOTAL']['ORDER_TOTAL_PRICE_FORMATED']?></a>
                                </div>
                            </div>
                            <p class="checkout__personal">
                                 <?php $APPLICATION->IncludeFile("/include/user_consent.php"); ?>
                            </p>
                        </div>
	...
	<script>
        ...
        $('#pay_order_button').click(function () {
            let pay_system_id = $('.checkout__pay_item.checkout__pay_active').attr('data-paysytem-id')
            let user_name = $('#bx-soa-properties input[name="ORDER_PROP_1"]').val()
            let user_email = $('#bx-soa-properties input[name="ORDER_PROP_2"]').val()
            let user_phone = $('#bx-soa-properties input[name="ORDER_PROP_3"]').val()
            let error_message = ''
            ...
            if (user_name.length == 0
                ...
                error_message = 'Не верный формат поля E-mail'
            } else if (!isPhoneValid(user_phone)) {
                error_message = 'Не верный формат поля Телефон'
            }
                ...
                $.post(
                    ...
                    function(data) {
                        ...
                        if (orderData.order_id != undefined)
                            window.location.href = `/personal/order/payment/?ORDER_ID=${orderData.order_id}&PAYMENT_ID=${pay_system_id}`
                    }
                )
            } else {
