...
foreach ($this->basketItems as $row)
{
	$rowData = array(
		...
		'BRAND' => isset($row[$this->arParams['BRAND_PROPERTY'].'_VALUE'])
			...
			: '',
    'CURRENT_PRICE' => number_format((float)$row['FULL_PRICE'] - (float)$row['DISCOUNT_PRICE'],0,'.',' ')
	);

  $db_res = CIBlockElement::GetByID($row['PRODUCT_ID']);
  if ($ar_res = $db_res->Fetch()) {
    if ($ar_res['IBLOCK_SECTION_ID'] == '1') {
      $rowData['SECTION_NAME'] = 'Дополнительные курсы для детей';
    } else {
      $rowData['SECTION_NAME'] = 'Дополнительные курсы для взрослых';
    }
  }
